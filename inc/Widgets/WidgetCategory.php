...
class WidgetCategory extends AbstractWidget
{
	...
	function form( $instance ) {
        ...
        <div class="nav-menu-widget-form-controls">
            <p>
                <label for="<?php echo $this->get_field_id( 'nav_menu' ); ?>"><?php _e( 'Select Menu', 'MXD' ); ?>:</label>
                <select id="<?php echo $this->get_field_id( 'nav_menu' ); ?>" name="<?php echo $this->get_field_name( 'nav_menu' ); ?>">
                    <option value="0">— Select —</option>
                    <?php foreach ($listNavMenus as $value): ?>
                        <option value="<?php echo $value->term_id; ?>" <?php selected( $nav_menu, $value->term_id ); ?>><?php _e( $value->name, 'MXD' ); ?></option>
                    <?php endforeach ?>
                </select>
            </p>
        </div>
